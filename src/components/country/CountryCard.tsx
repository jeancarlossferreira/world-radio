...
export function CountryCard({ country }: { country: Country }) {
  ...
  const countryName = localizeCountry(country.iso_3166_1);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <span className="font-medium text-sm block truncate">{countryName}</span>
      ...
      <button className="btn btn-ghost btn-circle btn-xs" onClick={handleLocate} title={t('action.locateOnMap')}>
        <MapPin size={16} />
      </button>
    </Link>
  );
